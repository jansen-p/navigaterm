...
class Tracker:
    def __init__(self, loc, loc_name, depth, init=False):
        ...
        if init:
            ...
            try:
                ...
                if not os.path.isfile(self.src_path + "data/" + loc_name):
                    print(
    ...
    # save dictionary
    def save(self):
        with open(self.data_path + self.loc_name, "wb") as fp:
            pickle.dump(self.data, fp, protocol=pickle.HIGHEST_PROTOCOL)

    # returns the saved dictionary
    def load(self):
        with open(self.data_path + self.loc_name, "rb") as fp:
            data = pickle.load(fp)
        return data
    ...
    def remove_dict(self):
        try:
            os.remove(self.data_path + self.loc_name)
    ...
    def find_hit(self, inp, match: Callable, match_file=False, response=False):
        ...
        for pth, score in sorted_tuples:
            ...
            if match_file:
                folder = self.current_location + "/" + pth
                files = [
                    f
